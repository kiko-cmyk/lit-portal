...
// Also carries `isB2B`: this is the one call the portal makes on EVERY entry
...
export const GET = withCustomer<{ subscriptions: Subscription[]; isB2B: boolean }>(async (req, ctx) => {
  ...
  const identity = devEmail
    ? { email: devEmail, tags: [] as string[] }
    : await shopifyAdmin.getCustomerEmailAndTags(ctx.customerId);
