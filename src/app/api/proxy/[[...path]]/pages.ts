  ...
  if (emailSent) {
    ...
    body = `
      <div style="text-align:center;padding:60px 20px;">
        ...
        <form method="POST" action="/apps/mi-portal/login" style="max-width:360px;margin:0 auto;">
...
export async function renderHomePage(email: string, sessionToken: string): Promise<string> {
  ...
  const body = `
    ...
    <div class="card">
      ...
      <a href="/apps/mi-portal/suscripciones${s}" class="btn btn-primary btn-sm">Gestionar</a>
    </div>
    ...
    <div class="card">
      ...
      <a href="/apps/mi-portal/recompensas${s}" class="btn btn-primary btn-sm">Ver mis puntos</a>
    </div>
    ...
    <div class="card">
      ...
      <a href="/apps/mi-portal/referidos${s}" class="btn btn-primary btn-sm">Mi código de referido</a>
    </div>

    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
      <a href="/apps/mi-portal/pedidos${s}" class="card" style="text-decoration:none;color:inherit;text-align:center;">
        <div style="font-size:24px;margin-bottom:8px;">📦</div>
        <h3 style="font-size:15px;">Mis Pedidos</h3>
      </a>
      <a href="/apps/mi-portal/contenido${s}" class="card" style="text-decoration:none;color:inherit;text-align:center;">
        <div style="font-size:24px;margin-bottom:8px;">🎬</div>
...
export async function renderSubscriptionsPage(email: string, sessionToken: string): Promise<string> {
  ...
  for (const sub of subs) {
    ...
    cardsHtml += `
      <div class="card">
        ...
        <div class="actions">
          ${sub.status === 'active' ? `
            <a href="/apps/mi-portal/api/action/skip/${sub.id}${s ? s + '&' : '?'}action=skip" class="btn btn-primary btn-sm">Saltar entrega</a>
            <a href="/apps/mi-portal/api/action/pause/${sub.id}${s ? s + '&' : '?'}action=pause" class="btn btn-secondary btn-sm">Pausar</a>
            <button onclick="showCancelModal('${sub.id}')" class="btn btn-danger btn-sm">Cancelar</button>
          ` : ''}
          ${sub.status === 'paused' ? `
            <a href="/apps/mi-portal/api/action/resume/${sub.id}${s ? s + '&' : '?'}action=resume" class="btn btn-primary btn-sm">Reanudar</a>
          ` : ''}
        </div>
      </div>
    `;
  ...
  const body = `
    ...
    <script>
      ...
      function doSkip() {
        if (currentSubId) window.location.href = '/apps/mi-portal/api/action/skip/' + currentSubId + '${s ? s + "&" : "?"}action=skip';
      }
      function doCancel() {
        if (currentSubId) window.location.href = '/apps/mi-portal/api/action/cancel/' + currentSubId + '${s ? s + "&" : "?"}action=cancel';
      }
