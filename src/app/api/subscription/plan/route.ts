...
export const PATCH = withCustomer<Subscription>(async (req, ctx) => {
  ...
  // Resolve target variant + selling_plan + detect what actually changed
  const targetBoxCount = body.boxCount ?? null;
  const targetFrequency = body.frequency ?? null;
  ...
  const newSellingPlanNumeric = targetFrequency ? SELLING_PLAN_BY_FREQUENCY[targetFrequency] : null;
  if (targetFrequency && !newSellingPlanNumeric) {
  ...
  const planChanged = newSellingPlanNumeric !== null && newSellingPlanNumeric !== mainItem.selling_plan_id;
  ...
  await seal.addItems(ids.sealSubscriptionId, [{
    ...
    // price omitted — let Seal use Shopify's variant price (avoids per-unit gotcha)
    ...(planChanged ? { sellingPlanId: newSellingPlanNumeric! } : {}),
  }]);
